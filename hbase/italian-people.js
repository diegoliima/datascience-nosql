...
let insertRecursiveIteractor = function(objectCollection, index, column, key){
    let commands = [];
    
    if(Array.isArray(objectCollection)){
        for (const keyObject in objectCollection) {
            if(objectCollection[keyObject] instanceof Object){
                commands = commands.concat(insertRecursiveIteractor(objectCollection[keyObject], index, column, key))
            }else{
                commands.push("put 'italians', '" + i + "', " + column + ":" + key + ":"+ keyObject +"', '" + objectCollection[keyObject] + "'")
            }
        }
    }else if(typeof(objectCollection) == 'object'){
        let keyCollection = Object.keys(objectCollection)
        for (let keyObject in keyCollection) {
            //console.log("put 'italians', '" + i + "', " + column + ":" + key + ":"+ keyCollection[keyObject] +"', '" + objectCollection[keyCollection[keyObject]] + "'")
            commands.push("put 'italians', '" + i + "', " + column + ":" + key + ":"+ keyCollection[keyObject] +"', '" + objectCollection[keyCollection[keyObject]] + "'")
        }
    }
...
commands.push("create 'italians', 'personal-data', 'app-data', 'professional-data', 'miscelaneous'")
commands.push("create 'italians', 'personal-data', 'app-data', 'professional-data', 'miscelaneous'")
commands.push("alter 'italians', {NAME=>'miscelaneous:cat', VERSIONS=>50}")
commands.push("alter 'italians', {NAME=>'miscelaneous:dog', VERSIONS=>50}")
commands.push("alter 'italians', {NAME=>'miscelaneous:movies', VERSIONS=>50}")
commands.push("alter 'italians', {NAME=>'miscelaneous:favFruits', VERSIONS=>50}")
...
for (let i = 0; i < 1000; ++i) {
    ...
    for (const column in person) {
        property = person[column]
        for (const key in property) {
            if(property[key] instanceof Object){
                commands = commands.concat(insertRecursiveIteractor(property[key], i, column, key).join("/n"))
            }else{
                commands.push("put 'italians', '" + i + "', " + column + ":" + key + "', '" + property[key] + "'")
            }
        }
    }
    ...
    const writeStream = fs.createWriteStream('italians.txt');
